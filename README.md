...
**Live demo:** https://real-time-chat-application-project.vercel.app
...
- **Auth** — email/password signup & login, JWT issued as both an `httpOnly` cookie and a Bearer token (so auth keeps working even when the browser blocks cross-site cookies, e.g. Incognito/Safari), session persistence, profile photo upload
...
The live deployment runs as a single Render **Web Service** built from the repo root (not the `render.yaml` blueprint) — the root `package.json` builds the frontend too and the backend serves it as a fallback, but the Vercel deployment in step 4 is what's actually used in production.

1. Push this repo to your GitHub account (already done if you're reading this from the repo).
2. Go to [render.com](https://render.com), sign up/log in, and click **New → Web Service**.
3. Connect your GitHub account and select this repository. Leave **Root Directory** blank (repo root).
...
6. Set these environment variables in the Render dashboard:
   - `NODE_ENV` = `production`
   - `MONGODB_URI` — from step 1. Use a database-user password with **letters and numbers only** (special characters like `@`, `:`, `/`, `!` break the connection string unless URL-encoded)
...
7. Deploy. Once live, copy your backend's URL, e.g. `https://your-backend.onrender.com`. Watch the deploy logs for `MongoDB connected: ...` to confirm the database is actually reachable before moving on.

> **Free tier note:** Render's free instances spin down after ~15 minutes of no traffic. The next request then takes ~50 seconds to wake it back up — this is expected, not a bug.
...
5. Add an environment variable: `VITE_API_URL` = `https://your-backend.onrender.com/api` (your Render URL from step 3, **with `/api` on the end** — a common mistake is leaving it off).
6. Deploy. Copy your frontend's URL, e.g. `https://your-app.vercel.app`.

> **`VITE_API_URL` is baked into the built files at build time**, not read at runtime. If you change it later, you must trigger a **new Vercel deploy** (Deployments → ⋯ → Redeploy) for the change to take effect.
...
- The backend and frontend are deployed on different domains (Render + Vercel), so the auth cookie is cross-site — Chrome Incognito and Safari/Firefox block third-party cookies by default, which breaks cookie-only auth. To handle this, login/signup also return the JWT in the response body; the frontend stores it and sends it as an `Authorization: Bearer <token>` header on every request. The `httpOnly` cookie is still set too, for same-origin/monolithic deployments.
